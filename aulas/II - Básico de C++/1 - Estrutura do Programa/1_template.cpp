# include <bits/stdc++.h>

using namespace std;

int main() {
    /* seu código vai aqui dentro =) */
}

/* ---------------------------------------------------------*/
/* -------------------- Template de C++ ------------------- */
/* ---------------------------------------------------------*/

/*
Todos os programas em C++ que faremos possuem esta estrutura (linha 1 
até 7). Um código em C++ é uma receita de bolo, lida da esquerda para a 
direita, de cima para baixo, em sequência, como uma série de instruções:

    -> Cabeçalho
    -> Início do Programa
        Instrução 1
        Instrução 2
        Instrução 3
            ...
        Instrução n
    -> Fim do Programa!

As linhas 1 e 3 são o cabeçalho. Não se preocupe com o significado disso, por
enquanto. São apenas rotinas padrão, que incorporaremos em todos os nossos 
códigos.

O que está dentro das chaves {} após main() é o que o programa executa de fato,
e vai depender do objetivo do código.

Após executar tudo que estiver na main, ele finaliza!
*/